...
@role_router.get("", tags=["role_list"])
async def role_list():
    service = RoleService()
    return service.list()

@role_router.post("", tags=["role_create"])
async def role_create(request: Request):
    service = RoleService()
    return await service.create(request)

@role_router.put("", tags=["role_update"])
async def role_update(request: Request):
    service = RoleService()
    return await service.update(request)

@role_router.delete("", tags=["role_delete"])
async def role_delete(request: Request):
    service = RoleService()
    return await service.delete(request)

@role_router.get("/{role_id}/permissions", tags=["role_permissions_list"])
